  ...
  return (
    <div className="p-4 animate-fade-in-up pb-24">
      <div className="flex flex-col items-center mb-8">
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center mb-4 ring-4 ring-gray-700 overflow-hidden">
          {user.photo_url ? (
      ...
      <div className="bg-gray-800 p-4 rounded-lg mb-8">
        <h3 className="text-md font-semibold text-gray-300 mb-2">Wallet Address</h3>
        {user.wallet_address ? (
          <p className="text-sm text-blue-300 bg-gray-700 px-3 py-2 rounded-lg truncate">{user.wallet_address}</p>
        ) : (
      ...
      <div className="space-y-4">
        ...
        {submissions.length > 0 ? (
          <ul className="space-y-3">
            {submissions.map(sub => (
              <li key={sub.id} className="bg-gray-800 p-4 rounded-lg flex items-center gap-4">
                <img src={sub.image_url} alt={sub.challenge_title} className="w-16 h-16 rounded-md object-cover bg-gray-700" />
                <div className="flex-1">
                  <p className="font-semibold text-white truncate">{sub.challenge_title}</p>
                  <p className="text-sm text-gray-400">Submitted: {new Date(sub.created_at).toLocaleDateString()}</p>
                </div>
              </li>
            ))}
          </ul>
